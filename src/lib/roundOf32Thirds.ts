export const THIRD_PLACE_WINNER_GROUPS = ['1A', '1B', '1D', '1E', '1G', '1I', '1K', '1L'] as const;

export type ThirdPlaceWinnerGroup = (typeof THIRD_PLACE_WINNER_GROUPS)[number];

export const ROUND_OF_32_THIRD_PLACE_MATCH_NUMBERS: Record<ThirdPlaceWinnerGroup, number> = {
  '1A': 79,
  '1B': 85,
  '1D': 81,
  '1E': 74,
...
export const THIRD_PLACE_COMBINATIONS: Record<string, Record<ThirdPlaceWinnerGroup, string>> = {
...
export function thirdPlaceCombinationKey(groups: string[]) {
  return [...new Set(groups)].sort((a, b) => a.localeCompare(b, 'en')).join('');
}

export function getThirdPlaceOpponentGroup(advancedThirdGroups: string[], winnerGroup: ThirdPlaceWinnerGroup) {
  const combination = THIRD_PLACE_COMBINATIONS[thirdPlaceCombinationKey(advancedThirdGroups)];
  return combination?.[winnerGroup] ?? null;
}

export function isThirdPlaceWinnerGroup(value: string): value is ThirdPlaceWinnerGroup {
  return (THIRD_PLACE_WINNER_GROUPS as readonly string[]).includes(value);
}