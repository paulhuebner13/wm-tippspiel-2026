...
function hasCompletePredictionValues(
  ...
  if (isKnockoutStage(match.stage) && home === away && !advanceTeamId) return false;
  return true;
}
...
export function ChangePredictionEditor({
  ...
  const [homeScore, setHomeScore] = useState(hideExistingTip ? '' : prediction?.predicted_home_score?.toString() ?? '');
  const [awayScore, setAwayScore] = useState(hideExistingTip ? '' : prediction?.predicted_away_score?.toString() ?? '');
  const [advanceTeamId, setAdvanceTeamId] = useState(hideExistingTip ? '' : prediction?.advance_team_id ?? '');
  ...
  const currentSubmitted = hasCompletePredictionValues(match, homeNumber, awayNumber, advanceValue);
  ...
  }, [advanceValue, homeNumber, match.id, awayNumber, saveKey, selectedProfile.id, startTransition, touched]);
